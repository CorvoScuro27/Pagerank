...
// utilizzata solamente per spiegare come si usano i parametri
void print_usage(const char *prog_name)
{
  fprintf(stderr, "Usage: %s [-k K] [-m M] [-d D] [-e E] [-t T] infile\n", prog_name);
}

// funzione utilizzate per assegnare (oppure no) un valore alle variabili definite sotto
void parse_arguments(int argc, char *argv[], int *K, int *M, double *D, double *E, int *T, char **infile)
{

  // assegno un valore di default alle variabili
  int opt;
  *K = DEFAULT_K;
  *M = DEFAULT_M;
  *D = DEFAULT_D;
  ...
  *T = DEFAULT_T;
  ...
  // scorro (se ci sono) gli argomenti passati al programma, e se ci sono gli assegno il valore specificato sulla riga di comando
  ...
  // la variabile globale optind contiene l'indice del primo argomento non elaborato
  if (optind < argc) 
  {
    *infile = argv[optind]; // l'argomento non elaborato è il file
  }
  else
  {
    termina("Input file is required.\n"); // termino se non passo un file
    print_usage(argv[0]);
  }
}


FILE *read_graph_dimensions(const char *infile, int *r)
{
  FILE *f = fopen(infile, "r");
  if (f == NULL)
  {
    termina("Error during the opening of the input file.");
  }

  int c, n;
  char line[256];

  // leggi gli archi dal file
  while (true)
  {
    ...
    // Controlla se la riga inizia con '%'
    if (line[0] == '%')
    {
    ...
    {
      break; // Esci dal ciclo dopo aver trovato e letto i valori
    }
  }
  return f; // Restituisci il file, resttuisco r per riferimento
}
